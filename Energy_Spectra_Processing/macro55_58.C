...
void macro55_58(){
    ...
    TH1D *histNa22 = (TH1D*)f->Get("run57.root");  histNa22->SetTitle("Na22");
    TH1D *histCs137 = (TH1D*)f->Get("run56.root"); histCs137->SetTitle("Cs137");
    TH1D *histCo60 = (TH1D*)f->Get("run55.root");  histCo60->SetTitle("Co60");
    TH1D *histMn54 = (TH1D*)f->Get("run58.root");  histMn54->SetTitle("Mn54");


    // We select all the regions of interest
    TCanvas *cNa22 = new TCanvas();  histNa22->Draw();   TF1 *fun1Na22 = fitGEL(histNa22, 600,800);
                                                         TF1 *fun2Na22 = fitGEL(histNa22, 1600,1900);
                                                         TF1 *fun3Na22 = fitGEL(histNa22, 2300, 2600); // sum peak
    TCanvas *cCs137 = new TCanvas(); histCs137->Draw();  TF1 *funCs137 = fitGEL(histCs137, 850,1000);
    TCanvas *cCo60 = new TCanvas();  histCo60->Draw();   TF1 *funCo60 = fitGEL2(histCo60, 1600,1880,2150); // (double gaussian)
                                                         TF1 *funCo60s = fitGEL(histCo60,3550,3950);   // sum peak 
    TCanvas *cMn54 = new TCanvas();  histMn54->Draw();   TF1 *funMn54 = fitGEL(histMn54, 1050,1250);
