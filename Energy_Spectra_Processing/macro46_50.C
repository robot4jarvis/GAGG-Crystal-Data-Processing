...
void macro46_50(){
    ...
    TH1D *histNa22 = (TH1D*)f->Get("run47.root");  histNa22->SetTitle("Na22");
    TH1D *histCs137 = (TH1D*)f->Get("run46.root"); histCs137->SetTitle("Cs137");
    TH1D *histCo60 = (TH1D*)f->Get("run49.root");  histCo60->SetTitle("Co60");
    TH1D *histMn54 = (TH1D*)f->Get("run48.root");  histMn54->SetTitle("Mn54");
    TH1D *histBACK = (TH1D*)f->Get("run50.root");  histBACK->SetTitle("BACK");

    // We select all the regions of interest
    TCanvas *cNa22 = new TCanvas();  histNa22->Draw();   TF1 *fun1Na22 = fitGEL(histNa22, 630,830);
                                                         TF1 *fun2Na22 = fitGEL(histNa22, 1700,2000);
                                                         TF1 *fun3Na22 = fitGEL(histNa22, 2400, 2700); // sum peak
    TCanvas *cCs137 = new TCanvas(); histCs137->Draw();  TF1 *funCs137 = fitGEL(histCs137, 870,1040);
    TCanvas *cCo60 = new TCanvas();  histCo60->Draw();   TF1 *funCo60 = fitGEL2(histCo60, 1500,1800,2100); // (double gaussian)
                                                         TF1 *funCo60s = fitGEL(histCo60,3400,3800);   // sum peak 
    TCanvas *cMn54 = new TCanvas();  histMn54->Draw();   TF1 *funMn54 = fitGEL(histMn54, 1100,1300);
    TCanvas *cBACK = new TCanvas(); histBACK->Draw(); TF1 *funBACK = fitGEL(histBACK, 2000,2400);


    // We have obtained the four fitted Functions -> We now extract the middpoints and sigmas
    double ADCchann[N] = {fun1Na22->GetParameter(0), fun2Na22->GetParameter(0), fun3Na22->GetParameter(0), funCs137->GetParameter(0),funCo60->GetParameter(0),funCo60->GetParameter(7), funCo60s->GetParameter(0),funMn54->GetParameter(0), funBACK->GetParameter(0)};
    double EADCchann[N] = {fun1Na22->GetParError(0), fun2Na22->GetParError(0), fun3Na22->GetParError(0), funCs137->GetParError(0),funCo60->GetParError(0),funCo60->GetParError(7), funCo60s->GetParError(0),funMn54->GetParError(0), funBACK->GetParError(0)};
    double sigma[N] =  {fun1Na22->GetParameter(1), fun2Na22->GetParameter(1),fun3Na22->GetParameter(1),funCs137->GetParameter(1),funCo60->GetParameter(1),funCo60->GetParameter(8), funCo60s->GetParameter(1),funMn54->GetParameter(1), funBACK->GetParameter(1)};
    double Esigma[N] =  {fun1Na22->GetParError(1), fun2Na22->GetParError(1),fun3Na22->GetParError(1),funCs137->GetParError(1),funCo60->GetParError(1),funCo60->GetParError(8), funCo60s->GetParError(1),funMn54->GetParError(1), funBACK->GetParError(1)};
    ...
    double Energy[N] = {511.0,  1274.537, 1785.537, 661.657, 1173.228, 1332.492,  2505.72  , 834.848, 1460.820};
    double EEnergy[N] ={   0.0,     0.007,   0.007,   0.003,    0.003,    0.004,    0.007,     0.003,    0.007};
    ...
    //TFile *fout = new TFile(fnameout + ".root", "UPDATE");
