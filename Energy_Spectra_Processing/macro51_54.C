...
void macro51_54(){
    ...
    TH1D *histNa22 = (TH1D*)f->Get("run54.root");  histNa22->SetTitle("Na22");
    TH1D *histCs137 = (TH1D*)f->Get("run51.root"); histCs137->SetTitle("Cs137");
    TH1D *histCo60 = (TH1D*)f->Get("run52.root");  histCo60->SetTitle("Co60");
    TH1D *histMn54 = (TH1D*)f->Get("run53.root");  histMn54->SetTitle("Mn54");


    // We select all the regions of interest
    TCanvas *cNa22 = new TCanvas();  histNa22->Draw();   TF1 *fun1Na22 = fitGEL(histNa22, 540,800);
                                                         TF1 *fun2Na22 = fitGEL(histNa22, 1500,1850);
                                                         TF1 *fun3Na22 = fitGEL(histNa22, 2150, 2500); // sum peak
    TCanvas *cCs137 = new TCanvas(); histCs137->Draw();  TF1 *funCs137 = fitGEL(histCs137, 740,1050);
    TCanvas *cCo60 = new TCanvas();  histCo60->Draw();   TF1 *funCo60 = fitGEL2(histCo60, 1400,1600,1950); // (double gaussian)
                                                         TF1 *funCo60s = fitGEL(histCo60,3050,3450);   // sum peak 
    TCanvas *cMn54 = new TCanvas();  histMn54->Draw();   TF1 *funMn54 = fitGEL(histMn54, 950,1250);
