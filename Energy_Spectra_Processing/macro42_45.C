...
void macro42_45(){
    ...
    TH1D *histNa22 = (TH1D*)f->Get("run43.root");  histNa22->SetTitle("Na22");
    TH1D *histCs137 = (TH1D*)f->Get("run45.root"); histCs137->SetTitle("Cs137");
    TH1D *histCo60 = (TH1D*)f->Get("run42.root");  histCo60->SetTitle("Co60");
    TH1D *histMn54 = (TH1D*)f->Get("run44.root");  histMn54->SetTitle("Mn54");


    // We select all the regions of interest
    TCanvas *cNa22 = new TCanvas();  histNa22->Draw();   TF1 *fun1Na22 = fitGEL(histNa22, 540,750);
                                                         TF1 *fun2Na22 = fitGEL(histNa22, 1500,1800);
                                                         TF1 *fun3Na22 = fitGEL(histNa22, 2150, 2450); // sum peak
    TCanvas *cCs137 = new TCanvas(); histCs137->Draw();  TF1 *funCs137 = fitGEL(histCs137, 740,1000);
    TCanvas *cCo60 = new TCanvas();  histCo60->Draw();   TF1 *funCo60 = fitGEL2(histCo60, 1400,1600,1900); // (double gaussian)
                                                         TF1 *funCo60s = fitGEL(histCo60,3050,3400);   // sum peak 
    TCanvas *cMn54 = new TCanvas();  histMn54->Draw();   TF1 *funMn54 = fitGEL(histMn54, 950,1200);
